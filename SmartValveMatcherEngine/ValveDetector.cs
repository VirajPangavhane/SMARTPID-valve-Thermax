...
public class ValveDetector
{
    ...
    public void MatchValves()
    {
        ...
        using (var tr = db.TransactionManager.StartTransaction())
        {
            ...
            var apiAttrs = SmartMarkDataLoader.LoadSmartMarkAttributes(ed);
            ...
            // Track entities that have already been assigned to prevent duplication
            var assignedEntities = new HashSet<Entity>();
            ...
            foreach (var tag in tagTexts)
            {
                ...
                var cluster = GetOwnedClusterForTag(ms, tr, tag.Position, allTagPositions, allowedSet, 25.0, 1, assignedEntities);

                if (cluster == null || cluster.Count == 0)
                {
                    ed.WriteMessage($"\n  No cluster found for tag: {tag.TextString}");
                    // Only mark with red square if not already processed
                    if (!IsSquareAlreadyMarked(tag.Position, ms, tr, 5.0)) // Increased tolerance
                    {
                        var squareSize = 7.0;
                ...
                var leaders = cluster.OfType<Leader>().ToList();
                var extras = new List<DBText>(); // don’t pull other valve tags

                string valveType = PatternMatcher.MatchGeometryToPatternStrict(lines, circles, arcs, pls, solids, hatches, leaders, patterns);

                if (string.IsNullOrWhiteSpace(valveType))
                {
                    ed.WriteMessage($"  Unmatched valve at tag: {tag.TextString} (missing required geometry elements - ALL elements must be present)");
                    // draw red square...
                    continue;
                }
                ...
                Dictionary<string, string> extraAttrs = apiAttrs;
                ...
                BlockBuilder.CreateValveBlock(
                    ...
                    lines, circles, arcs, solids, hatches, pls, leaders, extras,
                    mergedAttrs,
                    ref createdCount
                );
            }
    ...
    bool IsSquareAlreadyMarked(Point3d position, BlockTableRecord ms, Transaction tr, double tolerance = 5.0)
    {
    ...
    private void RemoveRedSquareIfExists(Point3d position, BlockTableRecord ms, Transaction tr, double tolerance = 5.0)
    {
        foreach (ObjectId id in ms)
        {
            var obj = tr.GetObject(id, OpenMode.ForRead);
    ...
    HashSet<Entity> assignedEntities)
    {
        ...
        foreach (ObjectId id in ms)
        {
            ...
            if (!(ent is Line || ent is Circle || ent is Arc || ent is Polyline || ent is Solid || ent is Hatch || ent is Leader)) continue;

            // Skip entities that have already been assigned to another valve
            if (assignedEntities.Contains(ent)) continue;
        ...
        // Find the cluster that is closest to this tag
        ...
        foreach (var cluster in clusters)
        {
            ...
            // If this is the closest cluster to this tag, assign it
            if (myDist < bestDist)
            {
                bestDist = myDist;
                owned = cluster;
        ...
        // Add the selected entities to the assigned set to prevent duplication
        if (owned != null)
        {
